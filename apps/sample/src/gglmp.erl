...
initmapfunc() -> "var map;function "++presentmap()++"() {map = new google.maps.Map(document.getElementById('map'),{zoom: 15,mapTypeControl:false,fullscreenControl:true}); var xhr = new XMLHttpRequest(); xhr.open(\"POST\", \"https://www.googleapis.com/geolocation/v1/geolocate?key="++authkey()++"\", true); xhr.setRequestHeader(\"Content-type\", \"application/json\"); xhr.onreadystatechange = function () { if (xhr.readyState == 4 && xhr.status == 200) { map.setCenter(JSON.parse(xhr.responseText).location); } else {console.log(xhr.responseText);map.setZoom(5);map.setCenter({lat: 39.8782338, lng: -98.4390371});} }; xhr.send(\"{}\"); map.addListener('idle', function() { ws.send(enc(tuple(atom('client'),tuple(bin('idle'),bin(JSON.stringify(map.getCenter().toJSON())),bin(JSON.stringify(map.getBounds().toJSON())),number(map.getZoom())))));});}".
infoWindowContent() ->
