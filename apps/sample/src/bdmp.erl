...
body() ->
  ...
  [#panel{id=map},
   #panel{class=["fixed-action-btn"],
          style=["bottom: 25px; right: 48px;"],
          body=[#link{class=["btn-floating btn-large waves-effect waves-light red"],
                      postback=btn,
                      body=[#i{class=["material-icons"],body=["add"]}]}]}
  ].
...
infoWindowContent() ->
  ...
  wf:to_list(wf:render(#panel{id=wf:state(infowindow)})).
...
event(btn) ->
  ...
  wf:state(validt_content,crypto:strong_rand_bytes(4)),
...
event({client, {<<"idle">>,Center,Bounds,_Zoom}}) ->
  ...
  PosiPropIDs = [begin {ok,{Lat,Lng}} = geohash:decode(list_to_binary(HashL)), {Lat,Lng,IDpropL} end||{HashL,IDpropL}<-GeoCaches],lists:foreach(fun draw_markers/1, PosiPropIDs);
...
event(Event) ->
  wf:info(?MODULE,"~p-> Unknown Event: ~p~n",[self(),Event]).
